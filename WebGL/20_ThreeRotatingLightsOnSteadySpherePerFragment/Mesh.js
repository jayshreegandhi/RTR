...
function Mesh() {
    ...
    this.addTriangle = function(single_vertex, single_normal, single_texture) {
        ...
        for (i = 0; i < 3; i++) {
            for (
                j = 0;
                j < numVertices;
                j++ //for the first ever iteration of 'j', numVertices will be 0 because of it's initialization in the parameterized constructor
            ) {
                if (
                    isFoundIdentical(verts[j * 3], single_vertex[i][0], diff) &&
    ...
    isFoundIdentical = function(val1, val2, diff) {
        // code
        if (Math.abs(val1 - val2) < diff) return true;
        else return false;
    };
