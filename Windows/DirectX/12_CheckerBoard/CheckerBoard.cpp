...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Rectangle = NULL;
...
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_Texture_Rectangle = NULL;
ID3D11SamplerState *gpID3D11SamplerState_Texture_Rectangle = NULL;

D3D11_MAPPED_SUBRESOURCE mappedSubresource;
float rectangleVertices[18];
...
UINT createBitmapImage(long ,long ,byte *, UINT, byte**);
...
HRESULT initialize(void)
{
	...
	bufferDesc_VertexBuffer_Poition_Rectangle.ByteWidth = 18 * sizeof(float);
	...
	//texture
	D3D11_BUFFER_DESC bufferDesc_VertexBuffer_Texture_Rectangle;
	ZeroMemory(&bufferDesc_VertexBuffer_Texture_Rectangle, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer_Texture_Rectangle.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer_Texture_Rectangle.ByteWidth = sizeof(float) * ARRAYSIZE(rectangleTexcoord);
	bufferDesc_VertexBuffer_Texture_Rectangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer_Texture_Rectangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Texture_Rectangle,
		NULL,
		&gpID3D11Buffer_VertexBuffer_Texture_Rectangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() failed for vertex buffer texture Rectangle.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() succeeded for vertex buffer texture Rectangle.\n");
		fclose(gpFile);
	}

	//copy vertices in above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture_Rectangle, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, rectangleTexcoord, sizeof(rectangleTexcoord));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture_Rectangle, NULL);
	...
	//load textures
	hr = LoadD3DTexture(&gpID3D11ShaderResourceView_Texture_Rectangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::LoadD3DTexture() failed for Rectangle texture.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::LoadD3DTexture() succeeded for Rectangle texture.\n");
		fclose(gpFile);
	}
	...
	hr = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Texture_Rectangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateSamplerState() failed for Rectangle texture.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateSamplerState() succeeded for Rectangle texture.\n");
		fclose(gpFile);
	}
...
HRESULT LoadD3DTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, gpID3D11DeviceContext, (const uint8_t*)bitmapImage, (size_t)totalBitmapImageSize, NULL, ppID3D11ShaderResourceView, NULL);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateWICTextureFromMemory() failed for texture resource.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateWICTextureFromMemory() succeeded for texture resource.\n");
		fclose(gpFile);
	}
...
HRESULT resize(int width, int height)
{
	...
	gPerspectiveProjectionMatrix = XMMatrixPerspectiveFovLH(XMConvertToRadians(60.0f), (float)width / (float)height, 0.1f, 30.0f);
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Texture_Rectangle, &stride, &offset);

	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture_Rectangle);
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Texture_Rectangle);
	...
	rectangleVertices[6] = -2.0f;
	rectangleVertices[7] = -1.0f;
	rectangleVertices[8] = 0.0f;
	rectangleVertices[9] = -2.0f;
	rectangleVertices[10] = -1.0f;
	rectangleVertices[11] = 0.0f;
	rectangleVertices[12] = 0.0f;
	rectangleVertices[13] = 1.0f;
	rectangleVertices[14] = 0.0f;
	rectangleVertices[15] = 0.0f;
	rectangleVertices[16] = -1.0f;
	...
	rectangleVertices[1] = 1.0f;
	rectangleVertices[2] = 0.0f;
	rectangleVertices[3] = 2.41421f;
	rectangleVertices[4] = 1.0f;
	rectangleVertices[5] = 1.41421f;
	...
	rectangleVertices[10] = -1.0f;
	rectangleVertices[11] = 0.0f;
	rectangleVertices[12] = 2.41421f;
	rectangleVertices[13] = 1.0f;
	rectangleVertices[14] = 1.41421f;
	rectangleVertices[15] = 2.41421f;
	rectangleVertices[16] = -1.0f;
	rectangleVertices[17] = 1.41421f;
...
void uninitialize(void)
{
	if (gpID3D11SamplerState_Texture_Rectangle)
	{
		gpID3D11SamplerState_Texture_Rectangle->Release();
		gpID3D11SamplerState_Texture_Rectangle = NULL;
	}

	if (gpID3D11ShaderResourceView_Texture_Rectangle)
	{
		gpID3D11ShaderResourceView_Texture_Rectangle->Release();
		gpID3D11ShaderResourceView_Texture_Rectangle = NULL;
	}
	...
	if (gpID3D11Buffer_VertexBuffer_Texture_Rectangle)
	{
		gpID3D11Buffer_VertexBuffer_Texture_Rectangle->Release();
		gpID3D11Buffer_VertexBuffer_Texture_Rectangle = NULL;
	}
...
UINT createBitmapImage(long imageWidth, long imageHeight, byte *imageColorData, UINT imageColorDataSize, byte **bitmap)
{
	...
	ZeroMemory((void *)&bitmapInfo, sizeof(BITMAPINFO));
	...
	*bitmap = (BYTE *)malloc(totalBitmapImageSize);
	ZeroMemory((void *)*bitmap, sizeof(totalBitmapImageSize));
