...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"		float3 light_direction_two = (float3)normalize(lightPosition_two - eyeCoordinates);" \
		...
		"		float tNorm_dot_lightDirection_two = max(dot(tNorm, light_direction_two), 0.0);" \
		...
		"		float3 reflectionVector_two = reflect(-tNorm_dot_lightDirection_two, tNorm_dot_lightDirection_two);" \
		...
		"		float4 specular_two = ls_two * ks * pow(max(dot(reflectionVector_two, viewerVector), 0.0), material_shininess);" \
		"		output.phong_ads_light = ambient_zero  + diffuse_zero + specular_zero + ambient_one  + diffuse_one + specular_one + ambient_two  + diffuse_two + specular_two;" \
		"	}" \
		"	else" \
		"	{" \
