...
HRESULT initialize(void)
{
	...
	bufferDesc_VertexBuffer_Normal_Cube.ByteWidth = sizeof(float) * ARRAYSIZE(cubeNormal);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Normal_Cube,
		NULL,
		&gpID3D11Buffer_VertexBuffer_Normal_Cube);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() failed for vertex buffer normal cube.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() succeeded for vertex buffer normal cube.\n");
		fclose(gpFile);
	}
	...
	//D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Normal_Cube, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
...
void display(void)
{
	...
	worldViewMatrix = scaleMatrix * rotationMatrix * translationMatrix;
	projectionMatrix = gPerspectiveProjectionMatrix;
	//XMMATRIX wvpMatrix = worldViewMatrix * projectionMatrix * gPerspectiveProjectionMatrix;
